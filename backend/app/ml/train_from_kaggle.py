...
def train_solar(data_mode: str) -> dict:
    ...
    prov.ModelCard(
        ...
        training_data_sources=[
            _src(
                "meenakshihihihihi/time-series-solar-irradiance-for-indian-cities",
                prov.REAL_BENGALURU,
                "Bengaluru (NASA-POWER-derived)",
            )
        ],
    ...
    print(
        f"[train_from_kaggle] Bengaluru GHI: R2={best_m['r2']} RMSE={best_m['rmse']} (REAL_BENGALURU, prod={prod})"
    )
    return {
        "agent": "kaggle_solar",
        "status": "TRAINED",
...
def train_cloud(data_mode: str) -> dict:
    ...
    if df is None or len(df) < 500 or df["irradiance_drop_risk"].nunique() < 2:
        return {
            "agent": "kaggle_cloud",
            "status": "SKIPPED",
            "reason": "insufficient/one-class cloud data",
        }
    feats = [
        "hour_sin",
        "hour_cos",
    ...
    xtr, xte, ytr, yte, sp = tu.chronological_split(
        df, feats, "irradiance_drop_risk", order_col="timestamp"
    )
    ...
    tu.save_pkl(
        ...
        "kaggle_cloud_risk_bengaluru_model.pkl",
    )
    prod = m["f1"] >= F1_PROD
    prov.ModelCard(
    ...
    print(
        f"[train_from_kaggle] Bengaluru cloud: F1={m['f1']} AUC={m.get('roc_auc')} (REAL_BENGALURU, prod={prod})"
    )
    return {
        "agent": "kaggle_cloud",
        "status": "TRAINED",
...
def train_load(data_mode: str) -> dict:
    ...
    if df is None or len(df) < 1000:
        ...
        return {
            "agent": "kaggle_load",
            "status": "SKIPPED",
            "reason": "INSUFFICIENT_REAL_KAGGLE_LOAD_DATA",
        }
    feats = [
        "hour_sin",
        "hour_cos",
    ...
    xtr, xte, ytr, yte, sp = tu.chronological_split(
        df, feats, "national_demand_mw", order_col="timestamp"
    )
    ...
    prov.ModelCard(
        ...
        training_data_sources=[
            _src(
                ...
                prov.REAL_INDIA,
                "India (National; Southern Region proxy)",
            )
        ],
    ...
    print(
        f"[train_from_kaggle] India load: R2={m['r2']} RMSE={m['rmse']} MW (REAL_INDIA, prod=False)"
    )
    return {
        "agent": "kaggle_load",
        "status": "TRAINED",
