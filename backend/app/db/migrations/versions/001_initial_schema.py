...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("panel_efficiency", sa.Float, server_default="0.18"),
        sa.Column("tilt", sa.Float, server_default="15.0"),
    ...
    op.create_table(
        "toy_weather_readings",
        sa.Column("id", UUID(as_uuid=True), primary_key=True),
        sa.Column("site_id", UUID(as_uuid=True), sa.ForeignKey("sites.id"), nullable=False),
        sa.Column("ts", sa.DateTime(timezone=True), nullable=False),
        sa.Column("irradiance_w_m2", sa.Float, nullable=False),
        sa.Column("cloud_cover_percent", sa.Float, nullable=False),
        sa.Column("temperature_c", sa.Float, nullable=False),
        sa.Column("humidity_percent", sa.Float, server_default="50.0"),
        sa.Column("wind_speed_mps", sa.Float, server_default="2.0"),
        sa.Column("rain_probability_percent", sa.Float, server_default="0.0"),
    ...
    op.create_table(
        ...
        sa.Column("source", sa.String(50), server_default="toy"),
    )
    ...
    op.create_table(
        ...
        sa.Column("penalty_rate_per_mw", sa.Float, server_default="1000.0"),
    )
    ...
    op.create_table(
        ...
        sa.Column("id", UUID(as_uuid=True), primary_key=True),
        sa.Column("site_id", UUID(as_uuid=True), sa.ForeignKey("sites.id"), nullable=False),
        sa.Column("schedule_window_id", UUID(as_uuid=True), sa.ForeignKey("schedule_windows.id"), nullable=True),
        ...
        sa.Column("fuzzy_risk_score", sa.Float, server_default="0.0"),
        sa.Column("fuzzy_risk_level", sa.String(20), server_default="LOW"),
    ...
    # Indexes for time-series queries (TimescaleDB-ready)
    op.create_index("ix_toy_weather_site_ts", "toy_weather_readings", ["site_id", "ts"])
...
def downgrade() -> None:
    ...
    op.drop_table("toy_weather_readings")
